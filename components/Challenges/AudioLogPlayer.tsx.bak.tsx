...
export default function AudioLogPlayer({ texto, macro, onPlayClick }: Props) {
  // Estado de reproducción: idle (sin reproducir) | playing | paused
  const [estado, setEstado] = useState<'idle' | 'playing' | 'paused'>('idle')
  // ¿La transcripción está visible? Depende del macronivel (ver más abajo).
  const [verTranscripcion, setVerTranscripcion] = useState(macro === 'A1')
  // Si el navegador no soporta síntesis de voz, lo avisamos y mostramos texto.
  const [soportado, setSoportado] = useState(true)
  // Voz en español seleccionada (puede tardar en cargar — ver useEffect).
  const vozRef = useRef<SpeechSynthesisVoice | null>(null)

  // En A1 la transcripción SIEMPRE se ve (andamiaje fonema-grafía).
  // En A2/B1/B2 va oculta por defecto con toggle (fomenta escucha autónoma).
  const transcripcionFija = macro === 'A1'
  ...
  return (
    <div className={styles.player}>
      ...
      {/* Transcripción: fija en A1, toggle en el resto. Si no hay soporte de voz,
          forzamos mostrarla para que el ejercicio siga siendo accesible. */}
      {!transcripcionFija && soportado && (
        <button
